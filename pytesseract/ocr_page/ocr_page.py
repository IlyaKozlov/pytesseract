...
from pytesseract.ocr_page.ocr_block import OcrBlock
...
class OcrPage:
    ...
    def text(self, confidence_threshold: float = 0) -> str:
        return "\n".join(blocks.text(confidence_threshold) for blocks in self.blocks)

    @staticmethod
    def from_dict(ocr_dict: Dict[str, List]) -> "OcrPage":
        ...
        for key in sorted(block2elements.keys()):
            ...
            blocks.append(OcrBlock.from_list(elements))
